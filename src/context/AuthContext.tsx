...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const createTenant = useCallback((data: Parameters<typeof TenantEngine.createTenant>[0]): Tenant => {
  ...
  const updateTenant = useCallback((tenantId: string, updates: Partial<Tenant>): Tenant | null => {
  ...
  const deleteTenant = useCallback((tenantId: string): boolean => {
  ...
  const setTenantStatus = useCallback((tenantId: string, status: TenantStatus): Tenant | null => {
  ...
  const setModuleEnabled = useCallback((moduleId: ModuleId, enabled: boolean) => {
  ...
  const setBusinessType = useCallback((newType: BusinessType) => {
