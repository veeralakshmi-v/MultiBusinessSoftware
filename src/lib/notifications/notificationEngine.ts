...
export class NotificationEngine {
  ...
  static getInitialSeeds(): NotificationRecord[] {
    const now = new Date();
    return [
      {
        id: 'notif-seed-1',
        event: 'INVOICE_CREATED',
        channel: 'WHATSAPP',
        channels: ['WHATSAPP', 'IN_APP', 'SMS'],
        recipient: { name: 'Customer #1001', mobile: '+919876543210' },
        title: 'Tax Invoice #INV-1001 Created',
        body: 'Invoice #INV-1001 for ₹1,250.00 processed successfully. WhatsApp & SMS alerts dispatched.',
        status: 'UNREAD',
        createdAt: new Date(now.getTime() - 1000 * 60 * 15).toISOString(),
        metadata: { invoiceNumber: 'INV-1001', amount: '1250.00' }
      },
      {
        id: 'notif-seed-2',
        event: 'LOW_STOCK',
        ...
        recipient: { name: 'Store Admin' },
        title: '🚨 Low Stock Threshold Alert',
        body: '3 inventory products have reached critical reorder level (<= 5 units remaining).',
        status: 'UNREAD',
        createdAt: new Date(now.getTime() - 1000 * 60 * 45).toISOString(),
        metadata: { itemName: 'Fast-Moving Stock', stockRemaining: '3' }
      },
      {
        id: 'notif-seed-3',
        event: 'STAFF_PUNCHED_IN',
        channel: 'IN_APP',
        channels: ['IN_APP'],
        recipient: { name: 'Operations Team' },
        title: 'Staff Geofenced Punch-IN Verified',
        body: 'Cashier clocked in at Main Store terminal with GPS geofence verified.',
        status: 'READ',
  ...
  static getNotifications(limit: number = 50): NotificationRecord[] {
    try {
      ...
      if (!raw) {
        const seeds = this.getInitialSeeds();
        try {
