          ...
          <div className="absolute bottom-3 left-1/2 -translate-x-1/2 z-30 flex items-center gap-1.5 sm:gap-2">
            {slides.map((_, i) => (
              <button key={i} onClick={() => setCurrent(i)}
                className={`rounded-full transition-all duration-300 ${i === current ? 'w-5 sm:w-6 h-1.5 sm:h-2 btn-theme-secondary' : 'w-1.5 sm:w-2 h-1.5 sm:h-2 bg-white/30 hover:bg-white/60'}`} />
            ))}
          </div>
        </>
      )}
...
const getDynamicCards = (secondaryColor: string) => [
  {
    id: 'billing',
    icon: Receipt,
    title: 'Billing POS',
...
function CardWatermark({ type, accent }: { type: string; accent: string }) {
  if (type === 'receipt') {
    return (
      <svg className="w-9 h-9 sm:w-11 sm:h-11 opacity-20 transition-opacity group-hover:opacity-35" viewBox="0 0 64 64" fill="none" stroke={accent} strokeWidth="2.5">
  ...
  if (type === 'users') {
    return (
      <svg className="w-12 h-9 sm:w-14 sm:h-11 opacity-20 transition-opacity group-hover:opacity-35" viewBox="0 0 80 64" fill="none" stroke={accent} strokeWidth="2.5">
...
export default function LandingPage() {
  ...
  useEffect(() => {
    const handleThemeChange = () => {
      const active = ThemeEngine.getThemeConfig();
      ThemeEngine.applyTheme(active);
      setThemeConfig(active);
  ...
  const secondaryBtnColor = themeConfig.secondaryBtnColor || '#C5A059';
  const cards = getDynamicCards(secondaryBtnColor);
  ...
  return (
    <div className="min-h-screen w-full overflow-y-auto bg-theme-primary text-white font-sans flex flex-col p-3 sm:p-4 md:p-6 relative">

      {/* Ambient background glows */}
      <div
        className="fixed top-0 left-1/4 w-[700px] h-[400px] rounded-full blur-[180px] pointer-events-none -translate-y-1/2 transition-all duration-500"
        style={{ backgroundColor: `${secondaryBtnColor}15` }}
      />
      ...
      <header className="flex-shrink-0 mb-3.5 sm:mb-4 px-4 sm:px-6 py-2.5 sm:py-3 rounded-2xl border border-white/[0.08] bg-theme-surface/80 backdrop-blur-xl shadow-xl flex items-center justify-between z-30 relative gap-3">
        <div className="flex items-center gap-2.5 sm:gap-3.5 min-w-0">
          {businessProfile.logoUrl ? (
            ...
            <div className="w-8 h-8 sm:w-9 sm:h-9 rounded-xl btn-theme-secondary flex items-center justify-center font-bold font-serif text-sm sm:text-base shadow-lg flex-shrink-0">
      ...
      <main className="flex-1 grid grid-cols-1 lg:grid-cols-[5fr_7fr] gap-3.5 sm:gap-5 lg:gap-6 relative z-10">

        {/* LEFT: Carousel Frame */}
        <div className="relative rounded-[1.25rem] sm:rounded-[1.5rem] lg:rounded-[1.75rem] overflow-hidden border border-white/[0.08] shadow-2xl bg-theme-surface flex items-center justify-center p-1 h-52 sm:h-60 lg:h-auto min-h-[200px] lg:min-h-[350px]">
          <div className="absolute inset-0 rounded-[1.25rem] sm:rounded-[1.5rem] lg:rounded-[1.75rem] ring-1 ring-inset ring-white/[0.05] pointer-events-none z-10" />

          {slides.length > 0 ? (
            <Carousel slides={slides} />
          ) : (
            <div className="flex flex-col items-center justify-center gap-3 sm:gap-4 text-center px-6 sm:px-10 py-6">
              <div className="relative">
                <div className="w-12 h-12 sm:w-16 sm:h-16 rounded-2xl bg-white/[0.04] border border-white/[0.08] flex items-center justify-center shadow-inner">
                  <ImageIcon className="w-5 h-5 sm:w-8 sm:h-8 text-gray-600" />
                </div>
                <div
                  className="absolute -inset-1.5 rounded-2xl border animate-ping opacity-30"
                  style={{ borderColor: secondaryBtnColor }}
                />
              </div>
              <div className="space-y-1">
                <p className="text-xs sm:text-sm font-semibold text-gray-300 tracking-wide">No Images Yet</p>
                <p className="text-[10px] sm:text-xs text-gray-500 leading-relaxed">
                  Upload photos via <span className="font-mono font-semibold" style={{ color: secondaryBtnColor }}>Settings → Landing Page</span>
                </p>
              </div>
              <div className="flex items-center gap-1.5 sm:gap-2 mt-1">
                <div className="w-5 sm:w-6 h-1.5 sm:h-2 rounded-full" style={{ backgroundColor: `${secondaryBtnColor}50` }} />
          ...
          {cards.map(card => {
            ...
            return (
              <div
                ...
                className="group relative rounded-[1.25rem] sm:rounded-[1.5rem] lg:rounded-[1.75rem] border border-white/[0.08] hover:border-white/20 bg-theme-surface hover:bg-theme-surface/90 p-4 sm:p-5 cursor-pointer transition-all duration-300 transform hover:-translate-y-1 hover:shadow-2xl overflow-hidden flex flex-col justify-between min-h-[140px] sm:min-h-[160px]"
              >
                {/* Top Row: Icon on left, Role Pill Badge on right */}
                <div className="flex items-start justify-between relative z-10">
                  <div
                    className="w-9 h-9 sm:w-11 sm:h-11 rounded-xl sm:rounded-2xl flex items-center justify-center transition-transform duration-300 group-hover:scale-110 shadow-md border"
                    style={{
                      backgroundColor: `${card.accent}20`,
                      borderColor: `${card.accent}40`
                    }}
                  >
                    <Icon className="w-4 h-4 sm:w-5 sm:h-5" style={{ color: card.accent }} />
                  </div>

                  {/* Soft Pill Badge */}
                  <div
                    className="px-2.5 py-0.5 rounded-full text-[10px] sm:text-[11px] font-semibold border tracking-wide"
                    style={isAdmin ? {
                      backgroundColor: `${secondaryBtnColor}20`,
                      borderColor: `${secondaryBtnColor}50`,
                ...
                <div className="my-2 sm:my-2.5 relative z-10">
                  <h3 className="font-bold text-sm sm:text-base text-white group-hover:text-theme-secondary transition-colors duration-300 leading-snug">
                    {card.title}
                  </h3>
                  <p className="text-[10px] sm:text-xs text-gray-400 group-hover:text-gray-300 mt-1 leading-relaxed transition-colors duration-300">
                    {card.desc}
                  </p>
                </div>
                ...
                <div className="flex items-end justify-between relative z-10 pt-0.5">
                  ...
                  <div className="w-7 h-7 sm:w-9 sm:h-9 rounded-full border border-white/10 bg-white/5 hover:bg-white/15 flex items-center justify-center text-white transition-all duration-300 group-hover:scale-110 group-hover:border-white/30 shadow-lg">
                    <ChevronRight className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-gray-300 group-hover:text-white transition-transform group-hover:translate-x-0.5" />
                  </div>
                </div>

                {/* Soft ambient background glow on hover */}
                <div
                  className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none rounded-[1.25rem] sm:rounded-[1.5rem] lg:rounded-[1.75rem]"
                  style={{ background: `radial-gradient(circle at 90% 10%, ${card.accent}15 0%, transparent 70%)` }}
                />
              </div>
            );
          })}
      ...
      <footer className="flex-shrink-0 mt-4 sm:mt-5 px-4 sm:px-6 md:px-8 py-2.5 rounded-xl border border-white/[0.05] bg-black/20 backdrop-blur-md flex flex-col sm:flex-row items-center justify-between gap-2 z-20 relative text-center sm:text-left">
        ...
        <div className="flex items-center gap-1.5">
          <Sparkles className="w-3 h-3" style={{ color: secondaryBtnColor }} />
          <span className="text-[10px] font-mono font-semibold" style={{ color: `${secondaryBtnColor}BB` }}>
            Powered by Foclen Software Pvt Ltd
          </span>
        </div>
      </footer>
    </div>
