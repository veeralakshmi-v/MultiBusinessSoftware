...
export default function Orders() {
  ...
  const fetchOrders = () => {
    ...
    const currentBizId = user?.businessId || localStorage.getItem('businessId') || 'biz-default-business';
    fetch('/api/orders' + (filter !== 'ALL' ? '?status=' + filter : ''), {
      ...
      .then(data => {
        if (Array.isArray(data) && data.length > 0) {
          const ids = new Set(loaded.map(o => o.id || o.orderNumber));
          data.forEach(d => {
            if (!ids.has(d.id) && !ids.has(d.orderNumber)) {
              loaded.push(d);
            }
          });
        }
        setOrders(filter !== 'ALL' ? loaded.filter(o => o.status === filter) : loaded);
      })
      .catch(() => {
        setOrders(filter !== 'ALL' ? loaded.filter(o => o.status === filter) : loaded);
      });
        ...
        <div className="flex items-center gap-1.5 bg-white border border-gray-200 p-1 rounded-xl overflow-x-auto no-scrollbar">
          <button
            ...
            className={cn(
              ...
              filter === 'ALL' ? "shadow-sm btn-theme-secondary" : "text-gray-600 hover:text-gray-900"
            )}
            style={filter === 'ALL' ? {
          ...
          {pipeline.map(step => (
            <button
              ...
              className={cn(
                "px-3 py-1.5 rounded-lg text-xs font-bold transition-all whitespace-nowrap cursor-pointer",
                filter === step.code ? "shadow-sm btn-theme-secondary" : "text-gray-600 hover:text-gray-900"
              )}
              style={filter === step.code ? {
                backgroundColor: 'var(--theme-btn-secondary)',
                color: 'var(--theme-btn-text)'
              } : {}}
            >
