...
export default function Login() {
  ...
  return (
    <div className="fixed inset-0 w-full h-full bg-[#F4F6FB] flex flex-col font-sans overflow-hidden">

      {/* Top Header Bar with Back to Home button */}
      <header className="px-6 py-3 flex items-center border-b border-gray-200/80 bg-white/80 backdrop-blur-xl z-10 shrink-0">
        <button
          onClick={() => navigate('/')}
          className="flex items-center gap-2 text-xs font-bold text-gray-700 hover:text-[#2563EB] px-3.5 py-1.5 rounded-xl transition-all hover:bg-blue-50 border border-gray-200/80 group cursor-pointer"
        >
      ...
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[800px] h-[350px] bg-gradient-to-b from-blue-100/50 via-purple-50/30 to-transparent blur-3xl pointer-events-none -z-10" />

      {/* Centered Main Form Container - No scroll */}
      <main className="flex-1 flex flex-col justify-center items-center px-4 py-2 sm:px-6 overflow-hidden">
        <div className="w-full max-w-md text-center shrink-0">
          <div className="flex justify-center">
            <div className="h-12 w-12 sm:h-14 sm:w-14 bg-[#2563EB] rounded-2xl flex items-center justify-center shadow-lg shadow-blue-500/25 text-white">
              <Receipt className="h-6 w-6 sm:h-7 sm:w-7 text-white" />
            </div>
          </div>
          <h2 className="mt-2.5 sm:mt-3 text-center text-2xl sm:text-3xl font-serif font-black tracking-tight text-[#0F172A]">
            Store Sign In
          </h2>
          <p className="mt-1 text-center text-[11px] font-mono font-bold text-gray-400 uppercase tracking-wider">
            Multi-Business Billing & POS Platform
          </p>
        </div>

        <div className="mt-4 sm:mt-5 w-full max-w-md shrink-0">
          <div className="bg-white py-5 sm:py-6 px-6 sm:px-8 shadow-xl rounded-2xl sm:rounded-3xl border border-gray-200/90 space-y-4">
            ...
            <form className="space-y-3.5" onSubmit={handleSubmit}>
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="block w-full rounded-xl border border-gray-300 bg-gray-50/60 pl-10 pr-4 py-2 text-xs text-gray-900 placeholder-gray-400 outline-none focus:bg-white focus:border-[#2563EB] focus:ring-2 focus:ring-blue-500/20 transition-all font-medium"
              ...
              <div>
                <button
                  ...
                  disabled={loading}
                  className={cn(
                    "flex w-full justify-center items-center gap-2 rounded-xl text-white py-2.5 sm:py-3 px-4 text-xs font-bold tracking-widest uppercase shadow-lg bg-[#2563EB] hover:bg-[#1D4ED8] shadow-blue-500/25 transition-all cursor-pointer",
