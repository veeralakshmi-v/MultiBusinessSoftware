...
app.put('/api/menu-items/:id', async (req, res) => {
  ...
  try {
    let targetCatId = categoryId;
    const catNameCandidate = (req.body.categoryName || req.body.category?.name || '').trim();
    if (targetCatId) {
      const existingCat = await prisma.category.findUnique({ where: { id: targetCatId } });
      if (!existingCat) {
        const nameToSearch = (catNameCandidate && !catNameCandidate.startsWith('cat-')) ? catNameCandidate : targetCatId;
        const foundByName = await prisma.category.findFirst({ where: { name: nameToSearch, businessId } });
        if (foundByName) {
          targetCatId = foundByName.id;
        } else if (nameToSearch && !nameToSearch.startsWith('cat-')) {
          try {
            const createdCat = await prisma.category.upsert({
              where: { name: nameToSearch.trim() },
              update: { slug: nameToSearch.trim().toLowerCase().replace(/\s+/g, '-') },
              create: {
                businessId,
                name: nameToSearch.trim(),
                slug: nameToSearch.trim().toLowerCase().replace(/\s+/g, '-'),
              }
            });
    ...
    } else if (catNameCandidate && !catNameCandidate.startsWith('cat-')) {
      const foundByName = await prisma.category.findFirst({ where: { name: catNameCandidate, businessId } });
      if (foundByName) {
        targetCatId = foundByName.id;
      } else {
        try {
          const createdCat = await prisma.category.upsert({
            where: { name: catNameCandidate.trim() },
            update: { slug: catNameCandidate.trim().toLowerCase().replace(/\s+/g, '-') },
            create: {
              businessId,
              name: catNameCandidate.trim(),
              slug: catNameCandidate.trim().toLowerCase().replace(/\s+/g, '-'),
            }
          });
          targetCatId = createdCat.id;
        } catch (e) { }
...
app.get('/api/orders', async (req, res) => {
  ...
  try {
    ...
    if (orders.length === 0 && businessId !== DEMO_BUSINESS_ID) {
...
app.get('/api/dashboard', async (req, res) => {
  ...
  try {
    ...
    if (allOrders.length === 0 && businessId !== DEMO_BUSINESS_ID) {
    ...
    const matchedBusinessId = allOrders.length > 0 ? allOrders[0].businessId : businessId;
    const totalCustomers = await prisma.customer.count({ where: { businessId: matchedBusinessId } });
    ...
    const dateOrders = allOrders.filter(o => o.createdAt.toISOString().slice(0, 10) === dateParam).map(formatOrder);
    const recentOrders = allOrders.slice(0, 30).map(formatOrder);
